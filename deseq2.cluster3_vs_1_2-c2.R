...
write.table(resSig2,file="cluster3_vs1_2_mean_20.xls",quote=F,sep="\t")
#filter 30
resSig2 <- resSig[which(resSig$log2FoldChange > 1 ),]
resSig2 <- resSig2[which(resSig2$baseMean > 30 & resSig2$lfcSE < 1),]
write.table(resSig2,file="cluster3_vs1_2_mean_30.xls",quote=F,sep="\t")
#filter 20
resSig2 <- resSig[which(resSig$log2FoldChange > 1 ),]
resSig2 <- resSig2[which(resSig2$baseMean > 40 & resSig2$lfcSE < 1),]
write.table(resSig2,file="cluster3_vs1_2_mean_40.xls",quote=F,sep="\t")
