...
database_homer<-read.table("homer.chromvar.motif.R3",header=T,sep="\t")
database_homer<-as.matrix(database_homer)
pwm_homer<-list()
for (i in seq(1,405,1)){
...
#chromvar analysis 
df<-readRDS("deseq2.mco.data.rds")
cluster.s<-read.table("cluster.new",header=F,sep="\t")
#peak_chr<-paste(df$chr,df$start,df$end,sep="_")
df4<-df[which(rownames(df) %in% cluster.s$V1),]

cluster<-read.table("sample",header=F,sep="")
cluster<-data.frame(overlap_cellline=cluster$V1,subtype_latest=cluster$V2)
cluster<-cluster[c(4:12),]
#cluster$subtype_latest<-c(rep("NSC",6),rep("GC1",3),rep("GC2",3),rep("GC3",3))
...
doc<-NULL
for (i in colnames(as.matrix(df3[,c(4:12)]))){
    num<-which(rownames(colData) == i )
    doc<-c(doc,num)
    }

colData2<-DataFrame(type=colData$type[doc],depth=as.numeric(colSums(df3[,c(4:12)])),row.names=rownames(colData)[doc])

rowRanges <- GRanges(df3$chr,IRanges(start=as.numeric(as.character(df3$start)),end=as.numeric(as.character(df3$end))),strand=rep("*",length(df3$chr)))
...
rse <- SummarizedExperiment(assays=SimpleList(counts=as.matrix(df3[,c(4:12)])),rowRanges=rowRanges, colData=colData2)
example_counts <- addGCBias(rse,genome =BSgenome.Mmusculus.UCSC.mm9)
...
#heatmap
filter<-deviations(dev)[order(variability$variability,decreasing=T),][1:50,]
merged_new2<-colData2
mergerd_new3<-merged_new2[order(colData2$type),]
...
col2<-col
col2[grep("GC1",col)]<-'#FFC0CB'
col2[grep("GC2",col)]<-'#808080'
col2[grep("GC3",col)]<-'#0000FF'
#col2[grep("NSC",col)]<-'#00FF00'
...
heatmap.2(target2,trace="none",scale="row",density="none",col=bluered(20),margins=c(10,7),labCol = FALSE,cexRow=0.5,ColSideColors=col2,Colv=T)
...
pheatmap(target2,cluster_rows=F,cluster_cols = F,border_color = "grey60",clustering_distance_cols="euclidean",scale="row",show_colnames= T,show_rownames = T,color = rev(color),annotation_col=mergerd_new3,clustering_distance_rows="euclidean", cex=1, clustering_method="complete",fontsize_col=4,breaks=breaks,annotation_colors = list(cluster=c("GC1"="#ae82b7","GC2"="#b1b376","GC3"="#8d96a5")),cellheight=10,cellwidth=10,fontsize = 8,fontsize_row =10 )
...
mergerd_new3$type <- factor(mergerd_new3$type)
target2<-filter[,order(colData2$type)]
#target3<-target2[,c(4,5,8,9,1,2,6,3,7)]


cluster1<-rownames(target2)[which(apply(target2[,1:3],1,min) > apply(target2[,4:9],1,max))]
cluster2<-rownames(target2)[which(apply(target2[,4:6],1,min) > apply(target2[,1:3],1,max) & apply(target2[,4:6],1,min) > apply(target2[,7:9],1,max))]
cluster3<-rownames(target2)[which(apply(target2[,7:9],1,min) > apply(target2[,1:6],1,max))]

b<-a[order(a$variability,decreasing=T),]

e<-variability[cluster2,]
f<-e[order(e$variability,decreasing=T),]

g<-variability[cluster3,]
h<-g[order(g$variability,decreasing=T),]

#write.table(cluster3.special,file="cluster3.special.txt",quote=F)
write.table(f,file="cluster2.special.txt",quote=F)
write.table(b,file="cluster1.special.txt",quote=F)
write.table(h,file="cluster3.special.txt",quote=F)

name<-c(cluster1,cluster2,cluster3)
...
mergerd_new3<-merged_new2[order(colData2$type),]
mergerd_new3$type <- factor(mergerd_new3$type)
target2<-filter[name,order(colData2$type)]

pdf("heatmap_deviation.dtf.pdf")
col<-as.character(mergerd_new3$type)
col2<-col
col2[grep("GC1",col)]<-'#FFC0CB'
col2[grep("GC2",col)]<-'#808080'
col2[grep("GC3",col)]<-'#0000FF'
#col2[grep("NSC",col)]<-'#00FF00'
heatmap.2(target2,trace="none",scale="none",density="none",col=bluered(20),margins=c(10,7),labCol = FALSE,cexRow=0.3,ColSideColors=col2,Colv=FALSE)
heatmap.2(target2,trace="none",scale="row",density="none",col=bluered(20),margins=c(10,7),labCol = FALSE,cexRow=0.3,ColSideColors=col2,Colv=T)
legend("topright",legend = unique(col),col = unique(col2),lty= 1, lwd = 5,cex=.7)
dev.off()
