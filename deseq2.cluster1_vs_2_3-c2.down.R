...
write.table(resSig2,file="cluster1_vs2_3_mean_20.down.xls",quote=F,sep="\t")
#filter 30
resSig2 <- resSig[which(resSig$log2FoldChange < -1),]
resSig2 <- resSig2[which(resSig2$baseMean > 30 & resSig2$lfcSE < 1),]
write.table(resSig2,file="cluster1_vs2_3_mean_30.down.xls",quote=F,sep="\t")
#filter 40 
resSig2 <- resSig[which(resSig$log2FoldChange < -1),]
resSig2 <- resSig2[which(resSig2$baseMean > 40 & resSig2$lfcSE < 1),] 
write.table(resSig2,file="cluster1_vs2_3_mean_40.down.xls",quote=F,sep="\t")
#counts filter
