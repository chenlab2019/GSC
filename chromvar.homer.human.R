...
#chromvar analysis 
df<-readRDS(input)
cluster.s<-read.table(input,header=F,sep="\t")
#peak_chr<-paste(df$chr,df$start,df$end,sep="_")
df4<-df[which(rownames(df) %in% cluster.s$V1),]

cluster<-read.table("new.cluster.50",header=F,sep="\t")
...
new<-c(cluster2.special,cluster1.special,cluster3.special)
target2<-filter[new,order(colData2$type)]
